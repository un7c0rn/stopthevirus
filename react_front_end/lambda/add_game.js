...
_defineProperty(Firestore, "add_game", async ({
  ...
  await Firestore.firestore.collection(`games`).doc(`${response.id}`).collection(`players`);
  await Firestore.firestore.collection(`games`).doc(`${response.id}`).collection(`teams`);
  await Firestore.firestore.collection(`games`).doc(`${response.id}`).collection(`tribes`);
