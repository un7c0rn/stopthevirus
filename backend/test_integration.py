...
class IntegrationTest(unittest.TestCase):
    ...
    def test_integration(self):
        ...
        # clean up
        gamedb.delete_collection(f'games/{game_id}/players')
        gamedb.delete_collection(f'games/{game_id}/tribes')
        gamedb.delete_collection(f'games/{game_id}/teams')
        gamedb.delete_collection(f'games/{game_id}/challenges')
        
        # add test challenges
        for i in range(10):
            gamedb.add_challenge(challenge=Challenge(
                name=f'Most creative mask challenge!',
                message=f'This challenge is simple. NOP! {i}',
            ))
        ...
        service = MatchmakerService(
            matchmaker=MatchMakerRoundRobin(), gamedb=gamedb, game_options=GameOptions(game_clock_mode=GameClockMode.ASYNC, game_wait_sleep_interval_sec=1))
        print('here!')
        ...
        for i in range(0, 10):
            gamedb.player(
                name=f'player-{i}', tiktok='unicorn', phone_number='+17742593288')
        print('here!')
